...
export default class LoginView extends React.Component {
  ...
  Login(){
    ...
    if(this.state.User.Username.length <= 0 || this.state.User.Password.length <= 0){
      ...
      let log = Card.Login('CardShop', 'CardShop');
      var status = log.status;
    }
  }
  Close = () => {
    if(!this.state.ToastView){
      this.setState({Toast: ''});
    }
  }
  componentDidUpdate(){
          ...
          <ImageBackground source={Image} resizeMode='cover' style={{width: '100%', height: '100%'}}>
            <Container>
              <Content padder contentContainerStyle={{flex:1, justifyContent: 'flex-end'}}>
                ...
                <Button block rounded iconLeft style={{marginTop: 40, marginBottom: 40, backgroundColor:'#b33b3c' }} onPress={this.Login.bind(this)}>
                  <Text>Login</Text>
                </Button>
              </Content>
